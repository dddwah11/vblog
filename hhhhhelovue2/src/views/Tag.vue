  ...
  <div>
    ...
    <el-card class="box-card">
      ...
      <el-table border  stripe :data="tableData" max-height="400" style="width: 100%;border">
        ...
        <el-table-column prop="name" label="标签名" width="110"></el-table-column>
...
<script>
import { findTagList, deletedById,reg } from "@/api/blog/tag";
export default {
  data() {
  ...
  methods: {
    async selectByPage() {
      const { data } = await findTagList(this.current, this.size);
    ...
    async addTag(){
      const { data:res } = await reg(this.tag);
      if(res.success===true){
        this.$notify.success({
          title:'操作成功',
          message:'添加成功'  
